...
*MultiIndex could possible carry 0 in the exponent term
...
struct MultiIndex : index_sequence<Is...> {
  ...
  //TD::index of the last non-zero element
  static constexpr std::size_t TD  = D -  last_non_zero(Is...);
  //TDV value of last non-zero element in this sequence
  static constexpr std::size_t TDV = TD==D?0:at<TD>(Is...);
};
...
struct size<GradedMonomialSequence<ORDER,DIM>>
    : std::integral_constant<std::size_t,
                             std::size_t(fmmtl::combination(ORDER+DIM, DIM))> {
};
...
struct _acc_nmk {
  template <typename K>
  //constexpr typename std::enable_if<(K{} <= N{})>::type operator()(K) {
  constexpr void operator()(K) {
    using NmK = decltype(N{} - K{});
    if ((K{} <= N{})) {
      get<N::I>(t) = get<N::I>(t) + get<K::I>(r) * get<NmK::I>(m);
    }
  };
 /* template <typename K>
  constexpr typename std::enable_if <!(K{} <= N{})>::type operator()(K) {};*/
...
struct _acc_npk {
  ...
  constexpr void operator()(K) {
    ...
    get<N::I>(t) = get<N::I>(t)+ get<K::I>(r) * get<NpK::I>(m);
  }
  ArrayT& t;
  const ArrayR& r;
  const ArrayM& m;
...
//GradedPolynomial is 
//a combination of items in GradedMonomial
//with different coefficients.
//hence can be represented by a array with the same length of
//GradedMonomial lists
