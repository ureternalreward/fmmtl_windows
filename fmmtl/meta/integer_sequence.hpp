...
//////////////
//concatenate sequences into one sequence
// intseq_cat
//#1
...
template<size_t N, typename _Ty = size_t>
struct typegen {
  using type = _Ty;
};
//#1
...
//#matches #1
struct at_c<N, index_sequence<ignore...>> {
  template <typename Nth>
  static constexpr Nth apply(typename typegen<ignore,Nth>::type..., Nth nth, ...)
  { return nth; }
};
...
constexpr const T at(T first, Xs... xs) {//-> const T {//decltype(*detail::at<N>::apply(&xs...)) {
  return detail::at_c<N>::apply<T>(first, xs...);
}
...
struct intseq_element<N,
  integer_sequence<T, I...> > {
  constexpr static T v = at<N>(I...);
